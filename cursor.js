// <![CDATA[
var colours = new Array("#ffff"); // colours of the hearts
var symbols = new Array(11089, 43612, 10038, 9734, 10030, 10022);
...
function mwah() {
  if (document.getElementById) {
    ...
    for (i = 0; i < hearts; i++) {
      ...
      var chosen_code = symbols[i % symbols.length];
      heart.appendChild(
        document.createTextNode(String.fromCharCode(chosen_code)),
      );
...
function herzle() {
  ...
  if (Math.abs(x - ox) > 1 || Math.abs(y - oy) > 1) {
    ...
    for (c = 0; c < hearts; c++)
      if (herzy[c] === false) {
        var chosen_code = symbols[c % symbols.length];
        herz[c].firstChild.nodeValue = String.fromCharCode(chosen_code);
